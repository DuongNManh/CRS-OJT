...
enum ApproverChartStatus {
  Pending = "Pending",
  Approved = "Approved",
  Rejected = "Rejected",
}

const ApproverChart: React.FC<ApproverChartProps> = React.memo(
  ({ userId, userRole, isActive }) => {
      ...
      try {
        ...
        const filteredData = Object.entries(response.data)
          .filter(([key]) => {
            ...
            return ["PENDING", "APPROVED", "REJECTED"].includes(upperKey);
          })
          .reduce(
            (acc, [key, value]) => {
              const formattedKey =
        ...
        const labels = Object.keys(filteredData) as Array<
          keyof typeof ApproverChartStatus
        >;
    ...
    return (
      <div className="animate__animated animate__zoomIn animate__faster p-6 w-full">
        <h2 className="text-5xl font-bold text-center mb-4 bg-gradient-to-r from-[#F27227] to-[#1169B0] text-transparent bg-clip-text">
          {t("chart.approver.title")}
        </h2>
