...
export const staffService = {
    ...
    async getStaffList(): Promise<ApiResponse<GetStaffResponse[]>> {
        try {
            const response = await axiosInstance.get<ApiResponse<GetStaffResponse[]>>(`${this.staffsEndpoint}`);
    ...
    async getStaffListPaging(pageNumber: number, pageSize: number, role: string, department: string): Promise<PagingResponse<GetStaffResponse>> {
        try {
            const response = await axiosInstance.get<ApiResponse<PagingResponse<GetStaffResponse>>>(`${this.staffsPagingEndpoint}?pageNumber=${pageNumber}&pageSize=${pageSize}&role=${role}&department=${department}`);
    ...
    async getStaffById(staffId: string): Promise<ApiResponse<GetStaffResponse>> {
        try {
            const response = await axiosInstance.get<ApiResponse<GetStaffResponse>>(`${this.staffEndpoint}/${staffId}`);
            return response.data;
        } catch (error: any) {
    ...
    async updateStaff(staffId: string, staffData: Partial<GetStaffResponse>): Promise<ApiResponse<GetStaffResponse>> {
        try {
            const response = await axiosInstance.put<ApiResponse<GetStaffResponse>>(`${this.staffEndpoint}/update?id=${staffId}`, staffData);
            return response.data;
        } catch (error: any) {
            const apiError = error.response?.data as ApiResponse<any>;
            if (apiError) {
    ...
    async deleteStaff(staffId: string): Promise<ApiResponse<any>> {
        try {
            const response = await axiosInstance.delete<ApiResponse<any>>(`${this.staffEndpoint}/delete?id=${staffId}`);
            return response.data;
        } catch (error: any) {
            const apiError = error.response?.data as ApiResponse<any>;
            if (apiError) {
    ...
    async checkEmailExists(email: string): Promise<boolean> {
        try {
            ...
            return staffList.some(staff => staff.email.toLowerCase() === email.toLowerCase());
        } catch (error: unknown) {
    ...
    async uploadImageAvatar(formData: FormData): Promise<IStaffDetails> {
        try{
            const response = await axiosInstance.post<IStaffDetails>(`${this.staffEndpoint}/upload-avatar`, formData);
            return response.data;
        } catch (error: any) {
            const apiError = error.response?.data as ApiResponse<any>;
            if (apiError) {
